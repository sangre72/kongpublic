...
class TelegramIO:
    ...
    def send_video_file(
        ...
        """로컬 비디오 파일을 멀티파트로 업로드하는 sendVideo(타임랩스 등)."""
        from pathlib import Path as _P
        ...
        if caption:
            fields["caption"] = caption[:1024]
        body, content_type = self._encode_multipart(
            fields, [("video", p.name, p.read_bytes())]
        )
        req = urllib.request.Request(
            f"{self.base}/sendVideo", data=body,
            headers={"Content-Type": content_type},
        )
        with urllib.request.urlopen(req, timeout=self.timeout + 120) as resp:
            payload = json.loads(resp.read().decode())
        if not payload.get("ok"):
