...
AUDIT_BUTTON = "🔍 주행검증(fable)"
APPLY_BUTTON = "🔧 보강 실행(opus)"

AUDIT_PROMPT = (
    "주행을 '사람 기준으로' 평가해라. 코드 품질 검사가 아니다.\n"
    "오너 기준: 목적지로 가려면 좌/우회전·유턴을 해야 하고, 차선을 지켜야 하고, "
    "필요할 때 차선을 바꿔야 한다. 실제 인간 세상과 같이 움직이는지 본다.\n"
    "1) 평가 항목은 네가 찾아서 정한다(운전에 필요한 것). 최소한 다음은 포함:\n"
    "   차로유지 / 조향 건전성 / 좌·우회전·유턴 / 차선변경 / 목적지 도달 / 속도 / 안전\n"
    "2) `python3 games/seoul-drive/eval_driving.py <초>` 로 실측한다. 추측 금지.\n"
    "   목적지를 실제로 넣고(검색→출발) 재측정해서 네비게이션도 본다.\n"
    "3) 네비게이션 말고 다른 평가축이 필요하면 스스로 추가한다.\n"
    f"4) 결과를 {AUDIT_FILE} 에 쓴다. 형식:\n"
    "   ## 검증일시 / ## 대상 / ## 평가 방법 / ## 확인된 것(실측값) / ## 문제점 / ## 보강할 점(우선순위순)\n"
    "5) 고치지는 마라. 찾아서 적기까지다.\n"
    "끝나면 몇 개 찾았는지 한 줄로 보고."
)

APPLY_PROMPT = (
    f"{AUDIT_FILE} 를 먼저 읽어라. 거기 적힌 '보강할 점'이 이번 작업 지시서다.\n"
    "· 우선순위 1번부터 차례로 실제 구현한다.\n"
    "· 하나 고칠 때마다 eval_driving.py 로 재측정해서 수치가 나아졌는지 확인한다.\n"
    "· 문서에 없는 걸 임의로 하지 마라. 문서가 기준이다.\n"
    "· 못 고치는 게 있으면 왜 못 고치는지 문서에 덧붙인다.\n"
    "끝나면 무엇을 고쳤고 수치가 어떻게 바뀌었는지 보고."
)
