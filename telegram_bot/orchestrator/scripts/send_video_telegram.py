#!/usr/bin/env python3
"""워커 → 유저 텔레그램 동영상 전송.
사용: python3 telegram_bot/orchestrator/scripts/send_video_telegram.py <file_path> ["caption"]
"""
import os, sys
...
if len(sys.argv) < 2:
    raise SystemExit("usage: send_video_telegram.py <file_path> [caption]")

file_path = sys.argv[1]
caption = sys.argv[2] if len(sys.argv) > 2 else None
tg = TelegramIO(token)
r = tg.send_video_file(chat_id, file_path, caption=caption)
