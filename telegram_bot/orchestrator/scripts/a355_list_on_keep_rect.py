#!/usr/bin/env python3
"""a_355: show list; delete 텍스트 only; keep/insert 사각형; SE drag; Google."""
...
def insert_rect(pid):
    ...
    click(shape["cx"], shape["cy"]); time.sleep(0.6)
    els = dump(pid)
    rect = find_menu_item(els, exact="직사각형")
    if not rect:
        notes.append("NO_직사각형 dump={}".format(
            [(e.get("label"), e.get("cx"), e.get("cy")) for e in els if e.get("role") == "AXMenuItem"][:20]))
        key("escape"); return False
...
def main():
    ...
    for _ in range(3):
        ...
        run(["screencapture", "-x", str(OUT / "slide_355.png")])
        write_notes(); return
    ...
    ensure_object_list(pid)
    els = dump(pid)
    notes.append("objs_before={}".format([(e.get("label"), e.get("cy")) for e in list_left(els)]))

    # Delete 텍스트 ONLY — never click 사각형, never cmd-a
    for i in range(15):
        els = dump(pid)
        objs = list_left(els)
        texts = [e for e in objs if (e.get("label") or "") == "텍스트"]
        rects = [e for e in objs if "사각" in (e.get("label") or "")]
        notes.append("loop{} texts={} rects={}".format(i + 1, len(texts), len(rects)))
        if not texts:
            break
        t = texts[0]
        # safety: never delete if label is 사각형
        if "사각" in (t.get("label") or ""):
            notes.append("ABORT_delete_would_hit_rect")
            break
        click(t["cx"], t["cy"]); time.sleep(0.35)
        # re-dump to verify selection isn't 사각형 somehow — we only clicked text row
        key("delete"); time.sleep(0.35)
        notes.append("deleted_text cy={}".format(t.get("cy")))
        # after delete check 사각형 still exists if it did
        els2 = dump(pid)
        r2 = [e for e in list_left(els2) if "사각" in (e.get("label") or "")]
        if rects and not r2:
            notes.append("RECT_LOST_after_delete_stop")
            # undo once to restore
            chord("cmd", "z"); time.sleep(0.4)
            notes.append("UNDO_rect_loss")
            break
    ...
    if rects:
        ...
        cy = (c0y + c1y) / 2
        for i in range(4):
            se_x, se_y = cx + 55 + i * 12, cy + 55 + i * 12
            drag(se_x, se_y, 1700, 1100)
            notes.append("drag_SE_{} {},{} -> 1700,1100".format(i + 1, se_x, se_y))
            time.sleep(0.5)
            els = dump(pid)
            rects = [e for e in list_left(els) if "사각" in (e.get("label") or "")]
            if rects:
        ...
        notes.append("NO_RECT_final")

    # Google — no cmd-a on whole slide; select text box content only after insert
    els = dump(pid)
    ...
    if ins:
        ...
        if tb:
            ...
            notes.append("text_box={},{}".format(tb["cx"], tb["cy"]))
    # type Google without selecting all document — triple-click field if needed
    text("Google"); time.sleep(0.35)
    gx = (c0x + c1x) / 2
    gy = c0y + (c1y - c0y) * 0.28
    click(gx, gy); time.sleep(0.2)
    ...
    notes.append("shot={}".format((OUT / "slide_355.png").stat().st_size if (OUT / "slide_355.png").exists() else 0))
