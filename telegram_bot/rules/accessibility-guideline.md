...
> a11y 중요 서비스 = 고령자·장애인·저시력도 주 사용자. build in from start, all UI (KWCAG 2.2 / WCAG 2.2 AA).

Applies: this project all web UI work. With design-guideline. (백엔드/봇 위주면 UI 산출물 생길 때.)

## 적용 강도 차등
- **Public front(이용자 영역) = strict(AA full)**: 랜딩·가입·신청·검색·예약·FAQ·약관·이용자 포털. 12 reqs all strict + axe 0.
- **Admin(내부 도구) = baseline**: 시맨틱·키보드·라벨·명도대비 basics 지키되 front-level(axe 0·full keyboard E2E)까지는 불요(내부 전문 사용자·과투자 지양).
...
12. **Lang**: `<html lang>` set.

## Verify
- Playwright + **axe-core** scan(serious 0). keyboard-only key flow E2E. theme preset contrast.
