...
★SCOPE-GATE(2026-09-03 generalized): applies ONLY to a web-app/DB screen-field surface using Korean-public-sector(MOIS) naming-convention — kong-bot=NONE(no React/form/DB-screen surface, cf code-structure.md §7). Reference/reuse-template for a future project needing government-standard field-naming; ¬active-gate today.

> ∀ data-fields **displayed/input-on-screen**: column-names follow **MOIS(행안부) 공통표준용어/표준단어**, apply-consistently to React-vars·form `id`/`name`. all screen-displayed-data = target.
...
2. **Dictionary in `src/lib/`** as module(e.g. `src/lib/naming.ts` or `field-dictionary.ts`).
   - Columns: Korean-label·standard-abbrev·camelCase·domain(type)·description. screen-labels·form-fields reference this dict.
...
4. **Data object keys**: mock·DTO·component-props field-names also unified-standard-camelCase(¬dup·¬arbitrary-abbrev).
5. **Neologism ∉ standard**(job-categories etc): combine-nearest-standard-words; if none → register-in-dict as in-house-standard-extension then display.
...
## Verify
- Form id/name·React-vars match standard-dict-abbrev(review/lint). ¬arbitrary-naming(e.g. `name1`, `email_addr`, `날짜`).
