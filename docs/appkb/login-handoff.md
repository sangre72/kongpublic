...
| 8 | ★QR-decode-and-send-URL is NOT universal(u_3627~3628 finding) — some sites' QR payload requires an actual CAMERA scan(app validates a live-scan event server-side, decoded-URL-open just re-lands on the app's own scan-screen — Upbit is this type). Naver's QR happened to accept direct URL-open as scan-equivalent, but that's Naver-specific, not a general QR property. Before reusing the decode-URL trick(cf RULE#2 exception) on a NEW site, verify it actually reaches a confirm-number/login-complete screen — if it just re-shows a scan-prompt, STOP retrying that method, look for a phone-approval alt(Kakao-style) instead. |
...
| **Upbit** VERIFIED ar_3628(u_3627~3630) — QR=CAMERA-SCAN-ONLY, ¬URL-openable(differs-from-Naver) | URL `upbit.com` login. a11y `카카오 로그인`/`QR 코드 로그인` buttons. | **default = Kakao-login**(phone-approval, no camera, cf a_3630) if account has it linked — NOT QR(QR payload=`/qr_code_to_app` app-camera-scan-target, opening the decoded URL directly just re-lands on app's OWN scan-screen, no confirm-number/direct-entry alt exists). shot Kakao-approval screen if shown. caption **`업비트 카카오 로그인 — 폰 카카오앱에서 승인`**. | 폰에서 카카오앱 승인. (QR only if Kakao unavailable — then true camera-scan needed, 2nd-device or user's own phone-camera-on-screen, not decode-and-open). | goal URL. |
