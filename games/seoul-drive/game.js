...
function gStartNode(x,y,ang){
  ...
  for(let si=0;si<GSEGS.length;si++){
    ...
    if(d>26*S) continue;                       // 너무 먼 도로는 후보 아님
    // 진행방향 일치도(-1~1). 일방통행이면 a→b 만, 양방향이면 절대값.
    const dot=(vx*fx+vy*fy)/L;
    const align = sg.o ? dot : Math.abs(dot);
    /* ★하드 컷 대신 소프트 페널티(ar_5034: GraphHopper heading_penalty 패턴).
       예전엔 align<0.55 를 그냥 버렸는데, 조건에 맞는 간선이 하나도 없으면
       경로 자체가 안 만들어졌다(실측: 일방통행 진행방향 끝점의 3.1%가 막다른 곳).
       업계 표준(Valhalla heading_tolerance 기본 60도 ≈ cos 0.5)을 기준선으로 쓰되,
       벗어나면 탈락이 아니라 점수를 깎는다. 후보가 아예 없어지는 일이 없다. */
    const TOL = 0.5;                           // cos(60도) — Valhalla 기본값
    const penalty = align < TOL ? (TOL - align) * 6.0 : 0;
    const score = align*2.2 - d/(6*S) - penalty;
    if(score>bs){
      bs=score;
...
function planTo(x,y){
  ...
  /* ★출발 시 차를 경로 위에 올려놓는다(u_5039).
     ...
     나가려는 이동이 되고, 하드 구속이 그걸 막는다 — 차가 가장자리에 낀 채
     핸들만 -0.65로 꺾고 제자리에서 사고만 반복했다(오너: "제자리 사고만 남").
     내비를 시작하는 순간은 '출발'이므로, 첫 경로점 위치·방향으로 정렬해준다.
     주행 중에는 절대 손대지 않는다(순간이동은 출발 1회뿐). */
  {
    const i=Math.min(auto.i, wp.length-1);
    const a=wp[i], b=wp[Math.min(i+1, wp.length-1)];
    const ang=Math.atan2(b.y-a.y, b.x-a.x);
    if(Math.hypot(a.x-me.x, a.y-me.y) < 120*S){   // 너무 멀면 건드리지 않는다
      me.x=a.x; me.y=a.y; me.ang=ang; me.v=Math.min(me.v, 6);
      me.offroad=0; me.cool=0.6;
    }
  }
  sync();
}
...
function driveAuto(dt){
     ...
     상태를 안 쌓으면 리셋돼도 즉시 정상 동작한다. */
  {
    ...
    for(let i=0;i<auto.wp.length;i++){
      ...
      if(dd > (200*S)*(200*S)) continue;          // 너무 먼 점은 후보 아님
      if(i<auto.wp.length-1 && (dx*fx+dy*fy) < 0) continue;   // 등 뒤는 제외(목적지는 예외)
      if(dd < bd){ bd=dd; bi=i; }
    }
    if(bi>=0) auto.i=bi;
  }
