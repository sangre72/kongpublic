...
function step(dt){
  ...
  /* ★건물(벽) 충돌 — 통과 불가. 이전 위치로 되돌리고 정지.
     ★2026-09-17 오너 실측: SK주유소 폴리곤이 도로(폭 6.5m) 위까지 덮어 그려져,
     차가 도로 중심선에서 1.59m(도로 안)에 있는데도 건물 충돌이 나 v=0으로
     영원히 갇혔다(109회 반복). u_5182 에서 같은 증상을 진단만 하고 고치지
     않았다. 원인은 OSM 폴리곤 좌표 오차/단순화이지 주행 실수가 아니므로,
     '도로 중심선 안'이면 건물 충돌 판정을 하지 않는다 — 실제 도로 위 주행을
     막을 이유가 없다. */
  const bi=(function(){ const r=onRoad(me.x,me.y); if(r&&r.ok&&r.s&&r.d<=r.s.roadW*0.5) return -1; return bldHit(me); })();
  if(bi>=0){
       ...
       3) 교차로 노드에서 roadW*1.5 안쪽이면 검사 자체를 끈다 — 회전 구간 */
  {
    ...
    if(ns && ns.s && !ns.s.o && ns.d <= ns.s.roadW*0.5 && !nearJunction(me.x,me.y,ns.s) && !insideOneWay(me.x,me.y) && !(auto.on&&auto.kt)){
      ...
      /* ★u_5253/u_5255 실측: 경로복귀를 부르는 갈래는 이 중앙선 구속이 지배적이다
         ...
         1.8m 라 차 왼쪽 모서리와 중앙선 사이 여유가 0.72m 뿐인데, 임계는 0.6m 고정
         이었다 — 0.12m 만 흔들려도 침범으로 잡혀 사실상 상시 위반이었다.
