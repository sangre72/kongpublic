...
_FACT_REPL = (
    (re.compile(r"\(\s*YES\b"), "(가능"),
    (re.compile(r"\(\s*NO\b"), "(불가"),
    (re.compile(r"\bANY app\b", re.I), "모든 앱"),
    (re.compile(r"screenshot\+mouse", re.I), "스크린샷+마우스"),
    (re.compile(r"\bscreenshot\b", re.I), "스크린샷"),
    (re.compile(r"\bmouse\b", re.I), "마우스"),
    (re.compile(r"browser-only", re.I), "브라우저 전용"),
    (re.compile(r"native desktop", re.I), "네이티브 데스크톱"),
    (re.compile(r"API-host-any", re.I), "API 호스트가 구현"),
    (re.compile(r"\bbrowser\b", re.I), "브라우저"),
    (re.compile(r"\bdesktop\b", re.I), "데스크톱"),
    (re.compile(r"\bOR\b"), "또는"),
)
...
def _strip_ar_dump(s: str) -> str:
    """Drop @AR header, markdown tables, USAGE — never send those to user."""
    out: list[str] = []
    for line in s.splitlines():
        t = line.strip()
        if t.startswith("@AR") or t.startswith("|") or t.startswith("USAGE:"):
            continue
        if re.match(r"^\[(STATUS|REF|WORKER|PARENT|CHAT_ID|DEP|UPDATED|HEARTBEAT)\]", t):
            continue
        out.append(line.rstrip())
    return "\n".join(out).strip()


def _topic_ko(ar_path: Path) -> str:
    m = re.match(r"ar_(\d+)_(.+)\.txt$", ar_path.name)
    if not m:
        return ar_path.stem
    words = [
        _SLUG_KO.get(w.lower(), w)
        for w in m.group(2).replace("-", "_").split("_")
        if w
    ]
    return " ".join(words) or f"작업 {int(m.group(1))}"


def _r_block(text: str) -> str:
    m = re.search(
        r"^R:\s*(.*?)(?=^(?:V:|F:|N:|USAGE:|## |\@AR)|\Z)",
        text,
        re.S | re.M,
    )
...
def _r_facts(r_block: str) -> list[str]:
    items: list[str] = []
    for line in r_block.splitlines():
        ...
        if not s or s.startswith("|") or s.startswith("@") or s.startswith("#"):
            continue
        s = re.sub(r"^u_\d+:\s*", "", s)
        items.extend(p.strip() for p in re.split(r"\s+·\s+", s) if p.strip())
        if len(items) >= 3:
            break
    return items[:3]


def _ko_fact(item: str) -> str:
    name, sep, rest = item.partition("=")
    body = rest if sep else item
    for pat, repl in _FACT_REPL:
        body = pat.sub(repl, body)
    body = re.sub(r"\(\s*가능\s+([^)]+?)\)", r"(\1 가능)", body)
    body = re.sub(r"\(\s*불가\s+([^)]+?)\)", r"(\1 불가)", body)
...
def telegram_user_summary(ar_path: Path) -> str:
    """Short Korean telegram body. Never dump @AR / EN table / full ar_ (K7).

    Prefer Hangul '## 결과' (auto-worker / timeout already write it).
    Else 1-3 facts from R: in Korean frame. ar_ file stays compressed-EN.
    """
    text = ar_path.read_text(encoding="utf-8")
    status = read_response_status(ar_path)
    verb = _STATUS_KO.get(status, f"처리 {status}")

    m = re.search(r"##\s*결과\s*\n(.+?)(?:\n##\s|\Z)", text, re.S)
    if m:
        ko = _strip_ar_dump(m.group(1))
        if ko and _has_hangul(ko):
            return ko[:1500]

    topic = _topic_ko(ar_path)
    facts = [_ko_fact(x) for x in _r_facts(_r_block(text))]
    lines = [f"{verb}: {topic}"]
...
def summarize_response(ar_path: Path) -> str:
    """Telegram-out summary. Does not dump English ar_ (see telegram_user_summary)."""
    return telegram_user_summary(ar_path)
