...
def main():
    ...
    if not els or not pid_h[0]:
        ...
        run(["screencapture", "-x", str(OUT / "slide_337.png")])
        write_notes(); print("FAIL app"); return
    pid = pid_h[0]
    notes.append("PID {}".format(pid))

    # require visible MAX window before any FS/object clicks
    for wi in range(5):
        els = dump(pid)
        wins = windows(els)
        notes.append("WIN_GATE{}={}".format(wi, wins))
        mw = max_doc(wins)
        if mw and (mw.get("w") or 0) >= 800:
            notes.append("visible_max_ok {}".format(mw))
            break
        notes.append("WINDOWS_empty_or_tiny re-open")
        run(["open", "-a", "Keynote"]); time.sleep(0.8)
        # try 윈도우 menu un-minimize
        if is_keynote(menus(els if els else dump(pid))):
            click(510, 20); time.sleep(0.45)
            els2 = dump(pid)
            item = next((e for e in els2 if e.get("role")=="AXMenuItem" and e.get("label") and (".key" in (e.get("label") or "") or "무제" in (e.get("label") or "") or "Keynote" in (e.get("label") or ""))), None)
            if item:
                notes.append("window_menu_pick {}".format(item.get("label")))
                click(item["cx"], item["cy"]); time.sleep(0.6)
            else:
                key("esc"); time.sleep(0.2)
        pid_h[0] = run(["pgrep", "-x", "Keynote"]).stdout.strip().split()[0]
        pid = pid_h[0]
    else:
        notes.append("FAIL_no_visible_keynote_window")
        run(["screencapture", "-x", str(OUT / "slide_337.png")])
        write_notes(); print("FAIL visible"); return
    ...
    for si in range(3):
        ...
        labs = menus(els)
        wins = windows(els)
        notes.append("shot_try{} menus={} wins={}".format(si, labs, wins))
        if is_keynote(labs) and max_doc(wins):
            notes.append("shot_keynote_visible_ok")
            break
        notes.append("shot_not_keynote_retry")
        time.sleep(0.4)
    ...
    notes.append("shot={}".format((OUT / "slide_337.png").stat().st_size if (OUT / "slide_337.png").exists() else 0))
